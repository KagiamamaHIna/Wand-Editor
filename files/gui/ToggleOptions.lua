...
function ToggleOptionsCB(_, _, _, iy, this_enable)
	...
	UI.MoveImagePicker("EditWandsEverywhere", PickerGap(1), iy + 40, 8, 0,
	...
	UI.MoveImagePicker("AlwaysDrawWandEditBox", PickerGap(2), iy + 40, 8, 0,
	...
	UI.MoveImagePicker("DisableSpellWobble", PickerGap(4), iy + 40, 8, 0,
	...
	UI.MoveImageButton("UpdateMod", PickerGap(0), iy + 60, "mods/wand_editor/files/gui/images/update_mod.png", nil,
		function()
		...
		function(click)
            local function RequestDefine()
				...
				local result = { https.request(CurrentMirror(address)) }
				if result[2] ~= 200 then
					result = { https.request(GitHub_Link(address)) } --如果镜像不行就尝试访问原始链接
				end
            ...
            if (click and UI.UserData["UpdateCheck"] == nil) or (click and UI.UserData["UpdateCheck"] == "error") then
				...
				ClickSound()
            elseif UI.UserData["UpdateThreadHandle"] and UI.UserData["UpdateCheck"] == "testing" then
	...
	UI.MoveImageButton("ModAbout", PickerGap(1), iy + 60, "mods/wand_editor/files/gui/images/about.png", nil, function()
		local _, _, hover = GuiGetPreviousWidgetInfo(UI.gui)
		if hover then
			UI.tooltips(function()
				...
				if Cpp.PathExists("mods/wand_editor/cache/avatar.png") then
					GuiImage(UI.gui, UI.NewID("AuthorAvatar"), 0, 0, "mods/wand_editor/cache/avatar.png", 1, 1 / UI.GetScale())
				end
			end, nil, 8)
