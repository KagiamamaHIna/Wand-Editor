...
namespace lua {
	...
	int lua_AbsPartialPinyinRatio(lua_State* L) {
		...
		std::vector<std::vector<std::string>> FirstPinyinBuf;
		...
		for (size_t i = 0; i < utf8s1.size(); i++) {//收集数据 
			if (pinyinData.HasPinyin(utf8s1[i])) {
				if (!temp.empty()) {//如果不为空,那就保存为一个单一的向量
					...
					FirstPinyinBuf.push_back(std::vector<std::string>{temp});
					temp.clear();
				}
				...
				FirstPinyinBuf.push_back(tempVec);
			}
		...
		stk = std::stack<std::pair<int, std::string>>();//用栈模拟递归函数行为
		stk.push({ 0, "" }); //初始索引为0，初始字符串为空
		...
		while (!stk.empty()) {
			...
			//将下一个索引的所有字符串加入栈中
			for (const auto& str : FirstPinyinBuf[index]) {
				stk.push({ index + 1, current + str });
			}
		}
